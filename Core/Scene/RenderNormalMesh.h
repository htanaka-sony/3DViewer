#ifndef RENDERNORMALMESH_H
#define RENDERNORMALMESH_H
...
public:
    ...
    const std::vector<Vertexf>&      displayVertices() const { return m_vertices; }
    ...
    std::vector<Vertexf>&      displayVertices() { return m_vertices; }
    ...
    void appendQuad(const Point3f& p0, const Point3f& p1, const Point3f& p2, const Point3f& p3, int get_seg_flg);
...
class CORE_EXPORT RenderEditableNormalMesh : public RenderableNode {
    DECLARE_META_RENDERABLENODE(RenderEditableNormalMesh, RenderNormalMesh)
protected:
    RenderEditableNormalMesh(RenderNormalMesh* mesh);
    ~RenderEditableNormalMesh();

    RenderEditableNormalMesh(const RenderEditableNormalMesh& other);
...
public:
    ...
    const std::vector<Vertexf>&      displayVertices() const { return originalMesh()->m_vertices; }
    ...
    std::vector<Vertexf>&      displayVertices() { return originalMesh()->m_vertices; }
    ...
    const std::vector<Vertexf>& displayEditVertices() const
    {
    ...
    std::vector<Vertexf>& displayEditVerticesTemp() { return !m_edit_buffer_2 ? m_edit_vertices_2 : m_edit_vertices; }
    ...
    void setDisplayEditData(std::vector<Vertexf>& verticies, std::vector<unsigned int>& indices)
    {
        if (m_edit_buffer_2) {
    ...
    RenderNormalMesh*       originalMesh() { return (RenderNormalMesh*)renderableObject(); }
    const RenderNormalMesh* originalMesh() const { return (const RenderNormalMesh*)renderableObject(); }
